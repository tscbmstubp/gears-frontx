...
**Classification is total over a stated universe.** A *candidate* is any package under a workspace root together with any unit declaring a template manifest — a template's root carries its manifest and need not be a package at all, so a universe defined over package manifests alone would omit the templates layer entirely. Every candidate must resolve to a layer or to one of the two stated categories, and an unresolved candidate is a failure, not a candidate that is skipped. Packages inside the content a template delivers are not candidates: on application they become content the receiving project owns, and the template delivering them is the candidate that answers for them. Totality is claimed over exactly this universe, and over members inside this repository only, for the reason stated above.
...
The membership form is confirmed by inspection: each layer's and each property's membership must be derivable from a stated check, and any concrete member list in the enforcement configuration must be generated or verified against the repository rather than authored. Totality is confirmed by a gate whose universe is the candidate universe this decision defines — so each template root is covered and the packages a template delivers as content are not — and which fails when a candidate resolves to no layer and to neither stated category; the present fallback for an unclassified package is unreachable and does not constitute such a gate.
...
Federated ownership is confirmed on a single member before generalization: a member registered as its own system node, with its own artifacts and its own coverage scope, validating without errors. Errors are the whole of the criterion because a member DESIGN cannot be warning-free for the reason recorded in the consequences above, so a warning-free criterion would be unsatisfiable rather than demanding.
