...
**The partition.** Three layers: **published libraries**, **templates**, and **projects orchestration**. Each layer is defined by the role its members fill, not by which packages currently fill it. The word *layer* is reserved for this partition and is not used for any other grouping of the same system.
...
**Within published libraries, two independent properties.** A library is **core** if it must remain UI-framework-agnostic. A library is **standalone** if it declares no intra-ecosystem package dependency, with the single exception of the type-substrate port. These are separate properties, not one combined test, and a library may hold either, both, or neither. Keeping them separate is what lets the agnostic-substrate guarantee apply to a library that legitimately depends on another library: welding the two would make the UI guarantee contingent on an unrelated question about dependencies, so a UI-agnostic library with one intra-ecosystem edge would fall out of *core* and lose that guarantee for a reason having nothing to do with its UI stack. A React-based component library holds neither property, which is permitted: `cpt-frontx-fr-ui-framework-agnostic` constrains the core framework and leaves the UI-stack choice to applications, templates, and libraries that are not core.
...
**Artifact ownership is federated.** The root owns the orchestration of the ecosystem and the contracts between layers. Each member owns the artifacts describing itself: a DESIGN and at least one FEATURE always; a PRD where the member has consumers and non-functional requirements of its own; never a DECOMPOSITION, whose purpose is organizing a large design into features and which for a single member would partition a handful of components into one feature. A member's artifacts may cite root requirement and design identifiers; the reverse — root artifacts citing a member's identifiers — would recouple the root to its members and is not permitted by this decision.
...
**A non-requirement, stated to prevent its invention.** No relation between templates exists or is required. Applying a template copies files that the receiving project may then change without constraint, so two templates that resemble each other are two independent templates and one is not derived from the other. The only durable link a template leaves behind is the per-application provenance record decided in `cpt-frontx-adr-project-provenance-record`.
...
**Scope.** This decision fixes the partition, the form membership takes, the two categories outside the layers, the requirement that classification be total, and the division of artifact ownership between root and member. It does not fix the identifier namespace, which is decided separately. It does not fix the concrete mechanical proxy by which each property is tested, nor the configuration through which member artifacts are registered — those are implementation of this decision and are owned by the enforcement configuration and the artifacts registry respectively. It does not restate the structure of each layer, which is DESIGN content.
...
* Bad, because federated ownership assumes a member's artifacts and the root's remain distinguishable, and the registry provides no such isolation: any member artifact may cite any root or sibling identifier and will validate. The citation direction this decision requires is therefore a stated constraint without mechanical enforcement until such a check is added.
...
* Bad, because the root PRD, DESIGN and DECOMPOSITION must be rewritten to describe the partition, a cost this decision incurs and does not itself pay.
...
Federated ownership is confirmed on a single member before generalization: a member registered as its own system node, with its own artifacts and its own coverage scope, validating clean. Evidence recorded at the time of this decision: a member DESIGN citing a root requirement identifier validates with no errors; per-member coverage scoping filters to that member's code; and required artifact kinds are enforced only where the member's node declares them, with the declaration-free form validating clean while enforcing nothing. The citation-direction constraint has no mechanical confirmation and is recorded above as a known non-guarantee.
...
* `cpt-frontx-adr-project-provenance-record` — Identified as the only durable link a template leaves behind, which is what makes the absence of any template-to-template relation a non-requirement rather than a gap.
